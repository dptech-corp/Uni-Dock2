...
from unidock2.torsion_library.utils import (
    ...
    get_torsion_mobile_atom_idx_list,
)
from unidock2.torsion_library.torsion_rule_matcher import TorsionRuleMatcher


class TorsionLibraryDriver(object):
    def __init__(self, mol, rotatable_bond_info_list, torsion_library_dict):
    ...
    def perform_torsion_matches(self):
        torsion_rule_matcher = TorsionRuleMatcher(
            self.mol, self.rotatable_bond_info_list, self.torsion_library_dict
        )

        torsion_rule_matcher.match_torsion_rules()
        self.matched_torsion_info_dict_list = (
            torsion_rule_matcher.matched_torsion_info_dict_list
        )
        ...
        for rotatable_bond_idx in range(self.num_rotatable_bonds):
            ...
            for matched_torsion_info_dict in self.matched_torsion_info_dict_list:
                current_rotatable_bond_info = matched_torsion_info_dict[
                    "rotatable_bond_info"
                ]
                if current_rotatable_bond_info == rotatable_bond_info:
                    torsion_atom_idx_list = matched_torsion_info_dict[
                        "torsion_atom_idx"
                    ]
                    self.torsion_atom_idx_nested_list[rotatable_bond_idx] = (
                    ...
                    self.enumerated_torsion_value_nested_list[rotatable_bond_idx] = (
                        matched_torsion_info_dict["torsion_angle_value"]
                    )
                    self.enumerated_torsion_range_nested_list[rotatable_bond_idx] = (
                        matched_torsion_info_dict["torsion_angle_range"]
                    )

            if not self.torsion_atom_idx_nested_list[rotatable_bond_idx]:
                torsion_atom_idx_list = get_torsion_atom_idx_tuple(
                    self.mol, rotatable_bond_info
                )
                self.torsion_atom_idx_nested_list[rotatable_bond_idx] = (
                    torsion_atom_idx_list
                )
    ...
    def identify_torsion_mobile_atoms(self, root_atom_idx):
        ...
        for rotatable_bond_idx in range(self.num_rotatable_bonds):
            rotatable_bond_info = self.rotatable_bond_info_list[rotatable_bond_idx]
            mobile_atom_idx_list = get_torsion_mobile_atom_idx_list(
                self.mol, rotatable_bond_info, root_atom_idx
            )
            ...
            torsion_atom_idx_list = self.torsion_atom_idx_nested_list[
                rotatable_bond_idx
            ]
            ...
            self.torsion_atom_idx_nested_list[rotatable_bond_idx] = (
                reordered_torsion_atom_idx_list
            )
            #########################################################################################
            #########################################################################################
